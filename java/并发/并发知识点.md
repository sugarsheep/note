...
> - 高优先级线程吞噬所有低优先级线程的CPU时间片
> - 线程被阻塞到一个等待进入同步块的状态，多个线程执行同步代码块时，一个线程执行时卡死了，导致其它线程都阻塞在该同步块
> - 等待的线程永远不被唤醒，调用了对象的wait()方法，但是没有其它线程调用notify()方法

#### 尽量避免饥饿问题

> - 设置合理的线程优先级，一般默认的就行
> - 使用锁代替synchronized（非公平锁）
...
> - 内存屏障  sfence（写屏障） mfence（全屏障，即读写屏障） lfence（读屏障）等系统原语
> - 锁总线

#### volatile如何解决指令重排序

> - java变量使用volatile修饰，在字节码上的体现是ACC_VOLATILE
>
> - JVM内存屏障：屏障两边的指令不会发生重排序，即**上面的代码肯定先于下面的代码执行**，但是屏障之前的指令可以重排，之后的也可以重排
>
>   ![1609168175868](images/1609168175868.png)
>
>   jvm规范要求对于volatile变量的读写需要添加如下的屏障
>
>   ![1609168494102](images/1609168494102.png)
>
> - hotspot对于内存屏障的实现
>
>   之所以实现不使用sfence、mfence、lfence等操作系统的内存屏障原语，是因为这些原语不是所有操作系统都有的，而lock指令对于大部分操作系统都是支持的
>
>   ![1609168711242](images/1609168711242.png)
>
> - 
...
- 字节码
  ...
  > - 将对象的引用赋值给栈上的变量
  >
  > 所以**double-check的单例模式，对于实例变量必须加volatile修饰**，防止指令重排序，即指令4和7，若指令7先于4执行，的此时变量的值已经不为null，但是对象的成员变量还只是默认值（还没有执行构造方法赋值），此时其它线程拿到对象的引用就会产生无法预料的结果

```java
public class com.sugar.spring_boot_demo.Test1228 {
  public com.sugar.spring_boot_demo.Test1228();
    Code:
       0: aload_0
  ...
  public static void main(java.lang.String[]);
    Code:
       0: new           #2 内存分配，对象的成员变量是默认值                 // class java/lang/Object
       3: dup
       4: invokespecial #1 调用对象的构造方法,成员变量进行赋值                 // Method java/lang/Object."<init>":()V
       7: astore_1  # 将对象的引用复制给o
       8: return
}
```
...
> 平时创建的对象都是强引用，只有当该对象没有任何引用指向的时候才会被回收
...
> GC时不会回收引用，当分配新的对象堆内存不足时就会回收软引用对象;它主要用于**缓存**

```java
//指定虚拟机参数-Xmx20m:设置最大堆内存为20m
public class MainTest {
...
> GC时就会被回收，不管是否有引用指向该对象

```java
public class MainTest {

    public static void main(String[] args) throws InterruptedException {
        WeakReference<A> s = new WeakReference<>(new A());
        System.out.println(s.get());
        System.gc();
        TimeUnit.MILLISECONDS.sleep(500);
        System.out.println(s.get());
...
输出结果
A create
com.alibaba.testable.demo.MainTest$A@60e53b93
A finalize
null
```
...
> ThreadLocalMap的Entry继承自WeakReference，并将key作为弱引用持有

![image-20201229170129132](${images}/image-20201229170129132.png)

#### 虚引用

> - 无法通过get获取对象，当对象被回收时，可以通过关联的Queue监测到，然后清理堆外内存
> - 用于管理直接内存，nio会使用该内存，GC监测Queue，当队列有元素时，就清理该元素和其关联的堆外内存
> - 应用：jdk NIO, netty的zero copy

![image-20201229101421612](${images}/image-20201229101421612.png)
